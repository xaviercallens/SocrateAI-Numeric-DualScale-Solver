...
\large Revision 3.1 --- Typographic and Cross-Reference Corrections}
...
When $\alpha' = 0$, Eq.~(3) reduces to the standard spectral
...
\begin{itemize}
    \item \textbf{Leray idempotence} (\texttt{leray\_idempotent}): the Fourier-space projector
          ...
          \texttt{EuclideanSpace}~$\mathbb{R}$~\texttt{(Fin}~$d$\texttt{)} via \texttt{field\_simp + ring}.
...
This projection is encoded in Eq.~(3) and formally certified by the
...
Table~1 reports the benchmark results averaged over the five independent
JHTDB temporal snapshots.

\begin{table}[ht]
    \centering
    \begin{tabular}{lp{3.2cm}p{2.8cm}p{3cm}}
        \toprule
        \textbf{Metric}
