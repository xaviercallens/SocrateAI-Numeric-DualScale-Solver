...
$\Omega(t) \le \Omega(0)$ and eliminating spurious numerical blow-ups.
...
The Ratchet loop, validated by \textbf{Pydantic runtime contracts} (H66--H70),
...
\textbf{Tier~B scaffolding} (\texttt{sorry} stubs); Tier~A proofs are a Phase~13 target.
(ii)~The Ratchet loop operates over a \textbf{1D scalar search space} per problem;
convergence in 1--3 iterations reflects this low dimensionality, not unconstrained
autonomous search.
(iii)~The $N{=}32$ ROM uses biharmonic hyper-dissipation, which may suppress
near-wall physics; results should be interpreted as \textbf{control-surrogate outputs},
not DNS-level solutions.
All 25 unit tests pass (2.34\,s), and a full SHA-256-sealed reproduction protocol is provided.
\end{abstract}
...
Invariants H66--H70 are encoded as Lean~4 theorem statements and enforced at
...
(\textbf{not statistically significant}).
...
\noindent However, the ROM provides valid \emph{directional} control utility:
$\tau_{\mathrm{ROM}}$ is strictly monotonically decreasing with $\alpha'$
(verified for $\alpha' \in [0.5, 2.0]$), so the ratchet loop correctly identifies
the \emph{direction} of improvement.
...
It must \textbf{NOT} be cited as a clinical safety claim. Absolute calibration
against the exact Couette solution has failed ($\rho = 0.52$, $p = 0.12$).
...
this should not be interpreted as evidence of general-purpose autonomous search.
...
\textbf{G3: Energy Yield} (Wind+BTMS) & +3.5\% / +8.0\% & +15.6\% / +31.9\% & $\mathbf{4.4\times / 4.0\times}$ \\
\textbf{G4: Surrogate Opt.\ / Directional Shear} (VAD) & 260\,Pa & 137.9\,Pa & $\mathbf{47\%}$ $\downarrow$ \\
\bottomrule
\end{tabular}%
}
\end{table}

All 4 gains are sealed in certificate \texttt{CERT-P12-AUTORESEARCH-A5B9217C06F6C669}.
